...
// ── Shared input/select styles for vision tool settings ──
...
export function ExtensionsTab({ onError }: { onError: (msg: string) => void }) {
  ...
  const handleSaveVisionConfig = async (
    patch: Partial<SetVisionConfigPayload>,
  ) => {
    setVisionCfgSaving(true);
    try {
      await setVisionConfig(patch);
      setVisionConfigState((prev) => (prev ? { ...prev, ...patch } : null) as VisionConfigResponse | null);
    } catch (err) {
      onError(err instanceof Error ? err.message : String(err));
    } finally {
      ...
      {hasVisionTool && visionConfig?.installed && (
        <div style={s.section}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "end", marginTop: 8 }}>
            <div style={{ flex: 1 }}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  void handleSaveVisionConfig({ provider: p, model: firstModel?.id ?? "" });
                }}
                style={{
                  width: "100%",
                  padding: "6px 8px",
            ...
            <div style={{ flex: 1 }}>
              ...
              <select
                ...
                onChange={(e) => {
                  void handleSaveVisionConfig({ provider: visionConfig.provider ?? "", model: e.target.value });
                }}
                style={{
                  width: "100%",
                  padding: "6px 8px",
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 10 }}>
            ...
            {/* Max dimension */}
            <div>
              <div style={{ fontSize: 10, color: "var(--text-dim)", marginBottom: 4 }}>Max dim (px)</div>
              <input
                type="number"
                min={100}
                max={5000}
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(v) && v >= 100 && v <= 5000)
                    void handleSaveVisionConfig({ maxDimension: v });
                }}
                style={inputStyle}
              />
            </div>
            ...
            {/* JPEG quality */}
            <div>
              <div style={{ fontSize: 10, color: "var(--text-dim)", marginBottom: 4 }}>JPEG quality</div>
              <input
                type="number"
                min={1}
                max={100}
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(v) && v >= 1 && v <= 100)
                    void handleSaveVisionConfig({ jpegQuality: v });
                }}
            ...
            {/* Reasoning effort */}
            <div>
              <div style={{ fontSize: 10, color: "var(--text-dim)", marginBottom: 4 }}>Reasoning effort</div>
              <select
                value={visionConfig.defaultReasoningEffort ?? "off"}
                onChange={(e) =>
                  void handleSaveVisionConfig({ defaultReasoningEffort: e.target.value as string })
                }
                style={selectStyle}
              >
