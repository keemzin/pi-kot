...
export const projectRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{
    ...
    async (req, reply) => {
      const { url, folderName, projectName, branch, token, insecureTls, targetPath: customTarget } = req.body;
      ...
      if (customTarget !== undefined) {
        ...
        targetPath = resolve(customTarget);
      } else {
        ...
        targetPath = resolve(config.workspacePath, folderName);
      }
      try {
