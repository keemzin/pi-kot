...
import { applyHunks, HunkStagingError } from "../hunk-stager.js";
...
function mapError(reply: FastifyReply, err: unknown): FastifyReply {
  ...
  reply.log.error({ err }, "unmapped git-operations error");
...
export const gitRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    "/git/commit/files",
    {
      schema: {
        description:
          "List files changed in a specific commit, with change " +
          "type (A/M/D/R). Uses git diff-tree under the hood.",
        tags: ["git"],
        querystring: {
          type: "object",
          required: ["projectId", "hash"],
          properties: {
            projectId: { type: "string", minLength: 1 },
    ...
    async (req, reply) => {
      return withGitCwd(
        ...
        reply,
        (cwd) => getCommitFiles(cwd, req.query.hash).then((files) => ({ files })),
      );
    },
  );
  ...
  fastify.get<{
    ...
    async (req, reply) => {
      return withGitCwd(
        ...
        req.query.worktreePath,
        reply,
        (cwd) => getCommitFileDiff(cwd, req.query.hash, req.query.path).then((diff) => ({ diff })),
      );
    },
  );
