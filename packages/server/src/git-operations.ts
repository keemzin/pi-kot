...
export interface CommitFileEntry {
  path: string;
  /**
   * Git change type: A(dded), M(odified), D(eleted), R(enamed),
   * C(opied), T(ype changed), U(nmerged), X(unknown).
   */
  changeType: "A" | "M" | "D" | "R" | "C" | "T" | "U" | "X";
}
...
export async function getCommitFiles(
  ...
  return stdout
    ...
    .split("\n")
    .filter(Boolean)
    .map((line): CommitFileEntry => {
      const [status, ...pathParts] = line.split("\t");
      const path = pathParts.join("\t");
      return {
        path,
        changeType: (status?.charAt(0) ?? "M") as CommitFileEntry["changeType"],
      };
    });
}
